...
public class GA {
    ...
    public void clear(){
        ...
        try {
            date = Calendar.getInstance();
            String text = "Log file cleared on " + date.getTime().toString() + "\n\n";
            file = new File("Log.txt");
            writer = new BufferedWriter(new FileWriter(file));
            writer.write(text);
        } catch (FileNotFoundException e) {
    ...
    */
    ...
    See the prey version
    */
    public Predator[] mutatePredatorPopulation(Predator[] population, int chance, int minMutStrength, int maxMutStrength) {
        ...
        for (int i = 0; i < population.length; i++) {
            for (int j = 0; j < 4; j++) {
                int lchance = rNumber.nextInt(1 + (100 - 1));
                if (lchance < chance) {
                    int r;
                    float range;
                    int addChance;
